...
### 🔁 変更
- [メイン画面] Ko-fi の バナーを削除して GitHub Sponsors のボタンに置き換え
- [メインメニュー/プロファイル管理パネル] 最終プレイ日時の表記を人間の読みやすい形式へ変更
- [メインメニュー] マップローテーションのゲームモード表記「ワイルドカード」に対応
- [アプリケーション設定パネル] Crowdin のURLを短縮リンクから直リンクへ変更
- [モザンビークAPI] API要求のリクエストヘッダーにユーザーエージェント (バージョン情報等) を追加
- Microsoft Store 向けの対応

### 🔧 修正
- マップローテーションのゲームモードが言語データに存在しない場合に表記が空白になる問題を修正
